...
void Multi1(int start, int end){
    ...
    for(int l = start; l < end; l++){
        for(int k = 0; k < 10; k++){
            for(int j = 0; j < 12; j++){
                for(int i = 0; i < 8; i++){
                    ymm[i] = __builtin_ia32_loadups256(&X[l][i*8+j*64]);
                    ymm[i+8] = __builtin_ia32_loadups256(&W_T[k][i*8+j*64]);
                }
                for(int i = 0; i < 8; i++){
            ...
            for(int i = 0; i < 2; i++){
                ymm[i] = __builtin_ia32_loadups256(&X[l][i*8+768]);
                ymm[i+8] = __builtin_ia32_loadups256(&W_T[k][i*8+768]);
            }
            for(int i = 2; i < 8; i++){
                ymm[i] = __builtin_ia32_loadups256(&zero[0]);
                ymm[i+8] = __builtin_ia32_loadups256(&zero[0]);
            }
            for(int i = 0; i < 8; i++){
                ymm[i] = __builtin_ia32_mulps256(ymm[i], ymm[i+8]);
            }
...
void Softmax(int start, int end){
    for(int x = start; x < end; ++x){
...
void *funct4(void *vptr){
    ...
    for(int l = start; l < end; l++){
        for(int k = 0; k < 10; k++){
            for(int j = 0; j < 937; j++){
                for(int i = 0; i < 8; i++){
                    ymm[i] = __builtin_ia32_loadups256(&X_T[l][i*8+j*64]);
                    ymm[i+8] = __builtin_ia32_loadups256(&y_hat_y_T[k][i*8+j*64]);
                }
                for(int i = 0; i < 8; i++){
                    ymm[i] = __builtin_ia32_mulps256(ymm[i], ymm[i+8]);
                }
            ...
            /*
            for(int i = 0; i < 4; i++){
                ymm[i] = __builtin_ia32_loadups256(&X_T[l][i*8+59968]);
                ymm[i+4] = __builtin_ia32_loadups256(&y_hat_y_T[k][i*8+59968]);
            }
            for(int i = 0; i < 4; i++){
                ymm[i] = __builtin_ia32_mulps256(ymm[i], ymm[i+4]);
            }
            for(int i = 1; i < 4; i++){
                ymm[0] = __builtin_ia32_addps256(ymm[0], ymm[i]);
            }
            __builtin_ia32_storeups256(scratchpad, ymm[0]);
            for(int i = 0; i < 4; i++) W_grad[l][k] += scratchpad[i];*/
