	...
	//leaf host
		...
		*/
		while(1){
			...
			fprintf(stderr, "(leaf)message from child:\n%d %d\n", person[0].id, person[1].id);
			if(person[0].id == -1){
//				fprintf(stderr, "LEAF FINISH!\n");
				break;
			}
			//fork player and exec(3)
			char newargv[2][100];
			for(int i = 0; i < 2; i++){
